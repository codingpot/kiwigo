...
때문에 친일의 산물은 아니다.`)
	wordInfos, _ := kb.ExtractWords(rs, 1 /*=minCnt*/, 5 /*=maxWordLen*/, -1000 /*=minScore*/, -1000 /*=posThreshold*/)
...
때문에 친일의 산물은 아니다.`)
	...
	text := scanner.Text()
	assert.Equal(t, `2008년에는 애국가의 작곡자 안익태가 1930년대에 독일 유학 기간 중 친일 활동을 했다는 사실이 밝혀졌다. 이후 안익태가 나치 독일 하의`, text)

	scanner2 := bufio.NewScanner(rs)
	scanner2.Scan()
	text = scanner2.Text()
	assert.Equal(t, `2008년에는 애국가의 작곡자 안익태가 1930년대에 독일 유학 기간 중 친일 활동을 했다는 사실이 밝혀졌다. 이후 안익태가 나치 독일 하의`, text)
}
